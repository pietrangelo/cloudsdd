...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// Registry is regional, so a pinned zone is a placement the user
			// asked for and will not get.
			name:     "build pipeline with zones",
			resource: pipelineResource(func(r *spec.Resource) { r.Scope.Zones = []string{"europe-west1-b"} }),
...
func TestResourceProgramDispatchesBuildPipeline(t *testing.T) {
	...
	findResource(t, recorded, cloudBuildTriggerToken)
	findResource(t, recorded, artifactRepositoryToken)
}
