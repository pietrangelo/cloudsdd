...
- [ ] Update `scripts/coverage-gate.sh`: add the `internal/judge` floor (98, from 98.6%). *(Added: the gate fails any package without a floor, so leaving this to the Phase 3 gate item would keep CI red for every item in between.)*
